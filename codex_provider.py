...
(plugins.agent_company.codex_exec_client.py) wraps it in an OpenAI chat-completion shape so
...
def register_codex_provider(ctx) -> None:
    """agent_company.register(ctx)에서 호출 — codex-exec provider를 전역 등록.
    ...
    agent_company의 단일 ``register(ctx)`` 진입점으로 모은다.
    """
